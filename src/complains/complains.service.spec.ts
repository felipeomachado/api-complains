...
describe('ComplainService', () => {
  ...
  beforeAll(async () => {
    ...
    service = module.get<ComplainsService>(ComplainsService);
  });
  ...
  describe('When search the number of Complains by geolocation', () => {
    ...
    it('should be count a existing complains in a specifc city', async () => {
      ...
      const complainsFound = await service.countComplainsQuery({cityId: '5f822543ad9cab1c444bcae5', companyId: undefined});
    ...
    it('should be count a existing complains in a specifc company', async () => {
      ...
      const complainsFound = await service.countComplainsQuery({cityId: undefined, companyId: '5f7f764cd54df8373c372df1'});
    ...
    it('should be count a existing complains in a specifc company and city', async () => {
      ...
      const complainsFound = await service.countComplainsQuery({cityId: '5f822543ad9cab1c444bcae5', companyId: '5f7f764cd54df8373c372df1'});
      expect(complainsFound).toEqual(1);
      expect(mockModel.find).toHaveBeenCalledTimes(1);
    });
  })
